...
// ----- Definições -----
#define WIFI_NAME   "AUTO-ELETRICA-NONATO"
...
void enviarValor(float num) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin(client, "http://192.168.18.55/receber.php");

    http.addHeader("Content-Type", "application/json");

    char json[100];
    sprintf(json, "{\"temperatura\":%.2f}", num);
...
void setup(void)
{
  ...
  server.serveStatic("/", LittleFS, "/Banana.html");
  server.on("/set", receberValor);
  server.on("/ligar", click);
...
void receberValor(){
  ...
  if (server.hasArg("graus")) {
    ...
    String valorTexto = server.arg("graus");

    Serial.print("Valor recebido: ");
    Serial.println(valorTexto);

    int graus = valorTexto.toInt();

    Serial.print("Convertido para inteiro: ");
    Serial.println(graus);
  }

  server.send(200, "text/plain", "Recebido com sucesso");
